...
function buildRunScript({
  ...
  return [
    ...
    `ISSUE_RUN_DIR=${shellQuote(issueRunDir)}`,
    `ISSUE_NUMBER=${shellQuote(issueNumber)}`,
    'STDOUT_PIPE="$ISSUE_RUN_DIR/stdout.pipe"',
    'STDERR_PIPE="$ISSUE_RUN_DIR/stderr.pipe"',
    '',
    'cd "$WORKTREE_PATH"',
    'set +e',
    'printf "\\n=== AIPR local Codex run: issue #%s ===\\n" "$ISSUE_NUMBER"',
    'printf "Worktree: %s\\n" "$WORKTREE_PATH"',
    'printf "Prompt: %s\\n" "$PROMPT_PATH"',
    'printf "Stdout log: %s\\n" "$STDOUT_PATH"',
    'printf "Stderr log: %s\\n\\n" "$STDERR_PATH"',
    ...
    'tee "$STDOUT_PATH" < "$STDOUT_PIPE" &',
    'stdout_tee_pid=$!',
    'tee "$STDERR_PATH" < "$STDERR_PIPE" >&2 &',
    'stderr_tee_pid=$!',
    `${command} < "$PROMPT_PATH" > "$STDOUT_PIPE" 2> "$STDERR_PIPE"`,
    'code=$?',
    'wait "$stdout_tee_pid"',
    'wait "$stderr_tee_pid"',
    'rm -f "$STDOUT_PIPE" "$STDERR_PIPE"',
